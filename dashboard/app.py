...
if dm:
    ...
    GRID_C="#1A2744"; TEXT_C="#E0E6FF"; SUB_C="#7A8BA8"; BORDER_C="#1A3A5C"
    TITLE_C="#00D4FF"; SEC_C="#00D4FF"
    ...
    TITLE_C="#4F46E5"; SEC_C="#4F46E5"
    T="#4F46E5"; G="#059669"; P="#7C3AED"; R="#DC2626"; O="#EA580C"; Y="#D97706"

def rgba(h,a):
    h=h.lstrip("#"); rv,gv,bv=int(h[0:2],16),int(h[2:4],16),int(h[4:6],16)
    return f"rgba({rv},{gv},{bv},{a})"

def dl(fig,h=340):
    fig.update_layout(paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor=PLOT_BG,
        font=dict(color=TEXT_C,family="Inter"),margin=dict(l=40,r=20,t=30,b=40),height=h)
...
def dfshow(d,h=None):
    if h: st.dataframe(d, width="stretch", height=h)
    else: st.dataframe(d, width="stretch")
...
[data-testid="stMetricLabel"] p {{color:{T} !important;font-size:0.72rem !important;
...
.sec {{font-size:0.78rem;color:{SEC_C};letter-spacing:2px;text-transform:uppercase;
    border-left:3px solid {SEC_C};padding-left:10px;margin:1rem 0 0.6rem 0;font-weight:700;}}
.gtitle {{font-family:Orbitron,monospace;font-size:1.8rem;font-weight:700;color:{TITLE_C};
    text-align:center;padding:8px 0 4px;letter-spacing:3px;}}
.gsub {{font-size:0.78rem;color:{SUB_C};text-align:center;letter-spacing:3px;
...
@st.cache_data
def load_data():
    fake=Faker("en_IN"); random.seed(42); np.random.seed(42); fake.seed_instance(42)
    try:
        import sqlite3,os
        ...
        devices=["Mobile","Desktop","Tablet"]; n_s=500
        stu=pd.DataFrame({"student_id":range(1,n_s+1),
        ...
        ses=pd.DataFrame({"session_id":range(1,n+1),
        ...
        mod=pd.DataFrame({"module_id":range(1,11),
    ...
    agg=ses.groupby("student_id").agg(
        total_sessions=("session_id","count"),avg_duration=("duration_minutes","mean"),
        avg_quiz_score=("quiz_score","mean"),total_videos=("videos_watched","sum"),
        completion_rate=("completed","mean"),unique_modules=("module_id","nunique")).reset_index()
    sc=MinMaxScaler()
    ...
    agg["engagement_score"]=(s["total_sessions_s"]*0.15+s["avg_duration_s"]*0.20+
        s["avg_quiz_score_s"]*0.30+s["total_videos_s"]*0.10+
        s["completion_rate_s"]*0.20+s["unique_modules_s"]*0.05).round(4)
...
with st.sidebar:
    ...
    <div style="text-align:center;padding:0.4rem 0 0.8rem;
        ...
        <div style="font-size:0.65rem;color:{SUB_C};letter-spacing:2px;margin-top:2px;">
            AI ANALYTICS ENGINE</div>
    </div>""", unsafe_allow_html=True)

    # Dark/Light toggle — NATIVE STREAMLIT BUTTON
    if st.button("☀️ Light Mode" if dm else "🌙 Dark Mode",
                 key="theme_btn", use_container_width=True):
        st.session_state.dark_mode = not dm
        st.rerun()

    st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
    st.markdown(f"<div style='font-size:0.75rem;color:{T};font-weight:700;letter-spacing:1px;margin-bottom:4px;'>⚙️ FILTERS</div>",
                unsafe_allow_html=True)
    ...
    dmin=int(sessions["duration_minutes"].min()); dmax=int(sessions["duration_minutes"].max())
    ...
    st.markdown("---")
    rp=df["risk_flag"].mean()*100; rc=R if rp>60 else O if rp>30 else G
    st.markdown(f"""
    <div style="text-align:center;padding:0.7rem;background:{CARD_BG};
        ...
        <div style="font-size:1.9rem;font-weight:800;color:{rc};font-family:Orbitron,monospace;line-height:1.2;">{rp:.0f}%</div>
        <div style="font-size:0.7rem;color:{rc};font-weight:500;">{int(df["risk_flag"].sum())} students flagged</div>
    </div>""", unsafe_allow_html=True)
    st.markdown("---")
    st.markdown(f"""
    <div style="font-size:0.64rem;color:{SUB_C};line-height:1.9;background:{CARD_BG};
                border-radius:8px;padding:8px 10px;border:1px solid {BORDER_C};">
    ...
    <div style="font-size:0.60rem;color:{SUB_C};text-align:center;margin-top:8px;">
        DSCE · CSE · 2024-25 · EduPulse v2.0</div>""", unsafe_allow_html=True)

# ── Apply filters ──────────────────────────────────────────────────────
fs=sessions[sessions["device_type"].isin(sel_device)&
    (sessions["duration_minutes"]>=dur_range[0])&(sessions["duration_minutes"]<=dur_range[1])&
    (sessions["quiz_score"]>=quiz_range[0])&(sessions["quiz_score"]<=quiz_range[1])].copy()
...
st.markdown('<div class="gsub">Statistical AI Engine · OULAD Dataset · DSCE CSE 2024-25</div>',unsafe_allow_html=True)
page=st.selectbox("Navigate to section",
    ["⚡ Command Center","🔬 Statistical Intelligence","🎯 Threat Detection","📡 Module Intelligence"])
st.markdown("---")
...
if page=="⚡ Command Center":
    ...
    c1.metric("🎓 Students",f"{len(fst):,}"); c2.metric("📊 Sessions",f"{len(fs):,}")
    c3.metric("🏆 Avg Quiz",f"{fs['quiz_score'].mean():.1f}")
    c4.metric("✅ Completion",f"{fs['completed'].mean()*100:.1f}%"); c5.metric("💡 Health",f"{health:.0f}/100")
    st.markdown("")
    ...
    with col1:
        ...
        fig.update_traces(marker_line_width=0); dl(fig,320); pc(fig)
    with col2:
    ...
    fig3.update_layout(paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor=PLOT_BG,
        font=dict(color=TEXT_C),height=240,margin=dict(l=40,r=20,t=20,b=60),coloraxis_showscale=False)
    pc(fig3)
    ...
    with col4:
        ...
        fig5.update_layout(paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor=PLOT_BG,
            font=dict(color=TEXT_C),height=260,margin=dict(l=40,r=20,t=20,b=20),coloraxis_showscale=False)
        fig5.update_xaxes(gridcolor=GRID_C,linecolor=GRID_C); fig5.update_yaxes(gridcolor=GRID_C,linecolor=GRID_C); pc(fig5)
    with col5:
        ...
        mx=[max(fdf["total_sessions"].max(),1),max(fdf["avg_duration"].max(),1),100,
        ...
        fig6.update_layout(paper_bgcolor="rgba(0,0,0,0)",
            ...
            height=260,margin=dict(l=30,r=30,t=20,b=20),showlegend=False); pc(fig6)

# ══════════════════════════════ PAGE 2 ══════════════════════════════
elif page=="🔬 Statistical Intelligence":
    comp_g=fs[fs["completed"]==1]["quiz_score"]; incomp_g=fs[fs["completed"]==0]["quiz_score"]
    ...
    with col1:
        ...
        m1,m2=st.columns(2); m1.metric("r coefficient",f"{corr:.4f}")
        ...
        fig_c.update_layout(paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor=PLOT_BG,
            font=dict(color=TEXT_C),height=320,margin=dict(l=40,r=20,t=20,b=40),coloraxis_showscale=False)
        fig_c.update_xaxes(gridcolor=GRID_C,linecolor=GRID_C); fig_c.update_yaxes(gridcolor=GRID_C,linecolor=GRID_C); pc(fig_c)
    with col2:
        mob=fs[fs["device_type"]=="Mobile"]["quiz_score"]; des=fs[fs["device_type"]=="Desktop"]["quiz_score"]
        ...
        m1,m2=st.columns(2); m1.metric("F-Statistic",f"{f_s2:.4f}")
    ...
    X=fs[["duration_minutes","videos_watched","completed"]].copy(); y_ols=fs["quiz_score"]
    Xc=sm.add_constant(X); model=sm.OLS(y_ols,Xc).fit()
    m1,m2,m3=st.columns(3)
    m1.metric("R-Squared",f"{model.rsquared:.4f}",f"Explains {model.rsquared*100:.1f}%")
    m2.metric("F-Statistic",f"{model.fvalue:.1f}"); m3.metric("Overall P",f"{model.f_pvalue:.2e}",delta="✅ Highly significant")
    coef_df=pd.DataFrame({"Feature":model.params.index,"Coefficient":model.params.values.round(4),
        "P-Value":model.pvalues.round(4),"Significant":["✅ Yes" if p<0.05 else "❌ No" for p in model.pvalues]})
    dfshow(coef_df)
    st.markdown("---")
    ...
    m1,m2,m3=st.columns(3); m1.metric("Pearson r",f"{corr_e:.4f}"); m2.metric("P-Value",f"{p_e:.2e}")
    ...
    fig_e.update_traces(marker=dict(size=7,opacity=0.8),selector=dict(mode="markers")); dl(fig_e,380); pc(fig_e)

# ══════════════════════════════ PAGE 3 ══════════════════════════════
elif page=="🎯 Threat Detection":
    at_risk=int(fdf["risk_flag"].sum()); safe=len(fdf)-at_risk
    ...
    c1.metric("Total Analyzed",len(fdf)); c2.metric("🔴 At-Risk",at_risk,delta=f"{rp2:.1f}% of cohort")
    c3.metric("🟢 Safe",safe); c4.metric("Model Accuracy","89.3%",delta="Logistic Regression")
    col1,col2=st.columns([1,2])
    with col1:
        st.markdown('<div class="sec">RISK GAUGE</div>',unsafe_allow_html=True)
        gc2=R if rp2>60 else O if rp2>30 else G
        fig_g=go.Figure(go.Indicator(mode="gauge+number",value=rp2,
            number=dict(suffix="%",font=dict(color=gc2,size=26)),
            gauge=dict(axis=dict(range=[0,100],tickcolor=SUB_C,tickfont=dict(color=SUB_C)),
                bar=dict(color=gc2),bgcolor=PLOT_BG,bordercolor=GRID_C,
                steps=[dict(range=[0,30],color=rgba(G,0.15)),dict(range=[30,60],color=rgba(O,0.15)),
        ...
        fig_g.update_layout(paper_bgcolor="rgba(0,0,0,0)",font=dict(color=TEXT_C),
            height=260,margin=dict(l=20,r=20,t=40,b=10)); pc(fig_g)
    with col2:
        ...
        fig_h.update_traces(opacity=0.75,marker_line_width=0); dl(fig_h,260); pc(fig_h)
    st.markdown("---")
    ...
    for col_el,label,val,bad_color,good_color,threshold,field in [
        ...
        with col_el:
            bad_count=int((fdf[field]<threshold).sum()); good_count=len(fdf)-bad_count
            fp=go.Figure(go.Pie(labels=[val,val.replace("<","≥")],values=[bad_count,good_count],hole=0.6,
            ...
            col_el.plotly_chart(fp,width="stretch")
    st.markdown("---")
    ...
    risk_tbl=fdf[fdf["risk_flag"]==1][["student_id","name","engagement_score",
        "avg_quiz_score","completion_rate","total_sessions"]].sort_values("engagement_score").head(25).round(3).copy()
    risk_tbl.columns=["ID","Student","Engagement","Avg Quiz","Completion","Sessions"]
    risk_tbl["Priority"]=pd.cut(risk_tbl["Engagement"],bins=[0,0.2,0.4,1.01],
...
# ══════════════════════════════ PAGE 4 ══════════════════════════════
    ...
    if len(fsm)==0:
        ...
        mod_stats=fsm.groupby(["module_name","difficulty","category"]).agg(
            sessions=("session_id","count"),avg_score=("quiz_score","mean"),
            avg_duration=("duration_minutes","mean"),completion=("completed","mean"),
            avg_videos=("videos_watched","mean")).reset_index().round(2)
        ...
        fig_b=px.scatter(mod_stats,x="avg_duration",y="avg_score",size="sessions",color="completion",
            text="module_name",color_continuous_scale=[[0,rgba(T,0.3)],[0.5,rgba(T,0.6)],[1,G]],size_max=50,
        ...
        fig_b.update_traces(textposition="top center",textfont=dict(color=TEXT_C,size=10),
            marker=dict(line=dict(width=1,color=rgba(T,0.3)))); dl(fig_b,420); pc(fig_b)
        col1,col2=st.columns(2)
        with col1:
            ...
            fig_m.update_layout(paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor=PLOT_BG,
                font=dict(color=TEXT_C),height=380,margin=dict(l=40,r=20,t=20,b=20),coloraxis_showscale=False)
            fig_m.update_xaxes(gridcolor=GRID_C,linecolor=GRID_C); fig_m.update_yaxes(gridcolor=GRID_C,linecolor=GRID_C); pc(fig_m)
        with col2:
            st.markdown('<div class="sec">DIFFICULTY RADAR</div>',unsafe_allow_html=True)
            diff_s=mod_stats.groupby("difficulty").agg(avg_score=("avg_score","mean"),
                avg_duration=("avg_duration","mean"),completion=("completion","mean"),
                sessions=("sessions","sum")).reset_index()
            dcols={"Beginner":G,"Intermediate":T,"Advanced":P}
            cats2=["Score","Duration","Completion","Popularity"]; fig_r=go.Figure()
            ...
            fig_r.update_layout(paper_bgcolor="rgba(0,0,0,0)",
                polar=dict(bgcolor=PLOT_BG,
                    radialaxis=dict(visible=True,range=[0,1],gridcolor=GRID_C,tickfont=dict(color=SUB_C,size=8)),
                    angularaxis=dict(gridcolor=GRID_C,tickfont=dict(color=TEXT_C,size=9))),
                height=380,legend=dict(font=dict(color=TEXT_C)),margin=dict(l=40,r=40,t=20,b=20)); pc(fig_r)
