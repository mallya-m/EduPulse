...
section[data-testid="stSidebar"] span { color: #A0AEC0 !important; }
[data-testid="stMetric"] { background-color: #0D1B2A; border: 1px solid #1A3A5C; border-radius: 10px; padding: 12px; }
[data-testid="stMetricLabel"] p { color: #00D4FF !important; font-size: 0.72rem !important; text-transform: uppercase; letter-spacing: 1px; }
...
.gtitle { font-family: Orbitron, monospace; font-size: 1.9rem; font-weight: 700; color: #00D4FF; text-align: center; padding: 8px 0; letter-spacing: 3px; }
.gsub { font-size: 0.8rem; color: #4A5568; text-align: center; letter-spacing: 3px; text-transform: uppercase; margin-bottom: 1rem; }
.sec { font-size: 0.8rem; color: #00D4FF; letter-spacing: 2px; text-transform: uppercase; border-left: 3px solid #00D4FF; padding-left: 10px; margin: 1rem 0 0.6rem 0; font-weight: 600; }
div[data-testid="stDataFrame"] { background-color: #0D1B2A; border-radius: 8px; }
...
def dl(fig, h=340):
    """Apply dark layout — no xaxis/yaxis keys to avoid conflict"""
    fig.update_layout(
...
with st.sidebar:
    ...
    <div style="font-family:Orbitron,monospace;font-size:1.1rem;color:{T};letter-spacing:3px;font-weight:700;">EDUPULSE</div>
    <div style="font-size:0.7rem;color:#4A5568;letter-spacing:2px;margin-top:4px;">AI ANALYTICS ENGINE</div>
    ...
    dur_range = st.slider("Duration (min)", dmin, dmax, (dmin, dmax))
    sel_diff = st.multiselect(
        "Module Difficulty",
        options=modules["difficulty"].unique().tolist(),
    ...
    background-color:#1A0A0A;border:1px solid {rc};border-radius:8px;">
    <div style="font-size:0.65rem;color:#4A5568;letter-spacing:2px;">RISK LEVEL</div>
    <div style="font-size:2rem;font-weight:700;color:{rc};font-family:Orbitron,monospace;">{rp:.0f}%</div>
    <div style="font-size:0.72rem;color:{rc};">students at risk</div>
    </div>""", unsafe_allow_html=True)
    st.markdown("---")
    st.markdown(f"""<div style="font-size:0.68rem;color:#2D3748;text-align:center;">
    DSCE · CSE · 2024-25<br>Data: OULAD (Kaggle)<br>v2.0 AI Edition</div>""", unsafe_allow_html=True)
...
if page == "⚡ Command Center":
    ...
    with col2:
        ...
        fig2.update_layout(
            ...
            annotations=[dict(text=f"{len(fs):,}<br>sessions",
                              font=dict(size=13,color=T), showarrow=False)]
        )
        st.plotly_chart(fig2, use_container_width=True)

    # Row 2 — hourly
    st.markdown('<div class="sec">STUDENT ACTIVITY BY HOUR</div>', unsafe_allow_html=True)
    ...
    fig3.update_xaxes(tickvals=list(range(24)),
                      ticktext=[f"{h}:00" for h in range(24)],
                      tickangle=45, gridcolor=GR, linecolor=GR)
    ...
    with col5:
        ...
        fig6 = go.Figure(go.Scatterpolar(
            ...
            line=dict(color=T,width=2), marker=dict(color=T,size=5)
        ))
        fig6.update_layout(
            paper_bgcolor="rgba(0,0,0,0)",
            polar=dict(
                bgcolor=BG,
                radialaxis=dict(visible=True,range=[0,1],gridcolor=GR,
                                tickfont=dict(color="#4A5568",size=8)),
                angularaxis=dict(gridcolor=GR,tickfont=dict(color="#A0AEC0",size=9))
            ),
...
# ══════════════════════════════════════════════════════════
    ...
    m3.metric("P-Value", f"{p_v:.2e}",
              delta="✅ Significant (p<0.05)" if p_v<0.05 else "❌ Not significant")
    ...
    with col1:
        ...
        fig_c = px.scatter(samp, x="duration_minutes", y="quiz_score",
                           color="quiz_score",
        ...
        fig_c.update_layout(
            ...
            font=dict(color="#A0AEC0"), height=320,
    ...
    fig_e = px.scatter(fdf, x="engagement_score", y="avg_quiz_score",
                       color="risk_flag",
...
# ══════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        fig_g = go.Figure(go.Indicator(
            ...
            title=dict(text="AT-RISK %", font=dict(color="#A0AEC0",size=12))
        ))
        fig_g.update_layout(
            paper_bgcolor="rgba(0,0,0,0)",
            font=dict(color="#A0AEC0"),
    ...
    with ca:
        ...
        f1 = go.Figure(go.Pie(
            labels=["Quiz < 50","Quiz ≥ 50"], values=[lq, len(fdf)-lq],
            hole=0.6, marker=dict(colors=[R,T]),
            textfont=dict(color="#A0AEC0")
        ))
        f1.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor=BG,
                         font=dict(color="#A0AEC0"), height=220,
                         margin=dict(l=10,r=10,t=10,b=10),
                         showlegend=True, legend=dict(font=dict(color="#A0AEC0",size=10)),
                         annotations=[dict(text="Quiz", font=dict(color=T,size=12), showarrow=False)])
        st.plotly_chart(f1, use_container_width=True)

    with cb:
        lc2 = int((fdf["completion_rate"]<0.3).sum())
        f2  = go.Figure(go.Pie(
            labels=["Comp < 30%","Comp ≥ 30%"], values=[lc2, len(fdf)-lc2],
            hole=0.6, marker=dict(colors=[O,G]),
            textfont=dict(color="#A0AEC0")
        ))
        f2.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor=BG,
                         font=dict(color="#A0AEC0"), height=220,
                         margin=dict(l=10,r=10,t=10,b=10),
                         showlegend=True, legend=dict(font=dict(color="#A0AEC0",size=10)),
                         annotations=[dict(text="Completion", font=dict(color=G,size=11), showarrow=False)])
        st.plotly_chart(f2, use_container_width=True)

    with cc:
        ls = int((fdf["total_sessions"]<5).sum())
        f3 = go.Figure(go.Pie(
            labels=["Sessions < 5","Sessions ≥ 5"], values=[ls, len(fdf)-ls],
            hole=0.6, marker=dict(colors=[P,Y]),
            textfont=dict(color="#A0AEC0")
        ))
        f3.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor=BG,
                         font=dict(color="#A0AEC0"), height=220,
                         margin=dict(l=10,r=10,t=10,b=10),
                         showlegend=True, legend=dict(font=dict(color="#A0AEC0",size=10)),
                         annotations=[dict(text="Sessions", font=dict(color=P,size=11), showarrow=False)])
    ...
    risk_tbl["Priority"] = pd.cut(
        risk_tbl["Engagement"], bins=[0,0.2,0.4,1.01],
...
# ══════════════════════════════════════════════════════════
    ...
    st.markdown("*Bubble size = sessions · Color intensity = completion rate · X = duration · Y = score*")
    ...
    with col1:
        ...
        fig_m = px.bar(ms, x="sessions", y="module_name", orientation="h",
                       color="avg_score",
                       color_continuous_scale=[[0,"#004D6B"],[1,T]],
                       text="sessions",
        ...
        fig_m.update_layout(
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor=BG,
            font=dict(color="#A0AEC0"), height=380,
            margin=dict(l=40,r=20,t=20,b=20),
            coloraxis_showscale=False
        )
    ...
    with col2:
        ...
        for _, row in diff_s.iterrows():
            ...
            col2r = dcols.get(row["difficulty"], T)
            fig_r.add_trace(go.Scatterpolar(
                r=vals2+[vals2[0]], theta=cats2+[cats2[0]],
                fill="toself", fillcolor=rgba(col2r,0.15),
                line=dict(color=col2r,width=2), name=row["difficulty"]
            ))
        fig_r.update_layout(
            paper_bgcolor="rgba(0,0,0,0)",
            polar=dict(
            ...
            height=380, legend=dict(font=dict(color="#A0AEC0")),
